    ...
    <div class="dialog-container">
      ...
      <div class="dialog-content">
        ...
        <div class="qrcode-authorized" v-else>
          ...
          <img
            class="icon-phone"
            src="../../assets/image/verify/qrcode-authorized.png"
            alt=""
          />
          <div class="tip">
...
function getQrcodeStatus() {
  verifyQrcodeStatus({ qr: qrCode.value })
    .then((res: ResponseType) => {
      // 失效
      if (res?.data?.qrCodeStatus === 21 && res?.data?.detailReason === 0) {
        qrcodeAuthorized.value = false;
        qrcodeInvalid.value = true;
      }
      // 等待扫码
      if (res?.data?.qrCodeStatus === 0 && res?.data?.detailReason === 0) {
        scanPolling(1000);
      }
      // 待确认 - 账号一致
      ...
      // 授权成功
      if (res?.data?.qrCodeStatus === 20 && res?.data?.detailReason === 0) {
        close();
      }
    })
    .catch(() => ({}));
