...
export default defineComponent({
  ...
  setup(props, { emit }) {
    ...
    // 上一页
    function prevChange(): boolean | undefined {
      if (current.value <= 1) {
        return false;
      }
    ...
    // 下一页
    function nextChange(): boolean | undefined {
    ...
    // 处理当前页改变
    function changePage(): boolean | undefined {
      emit('changPage', current.value);
      // 重置page数组
      pageList.value = [];

      // 当前页 = 显示页数，并且是最后一页
      if (
        current.value === displayPage.value &&
        totalPage.value === displayPage.value
      ) {
        for (let i = 1; i <= totalPage.value; i++) {
          if (i <= displayPage.value) {
            pageList.value.push(i);
      ...
      if (current.value >= displayPage.value) {
        ...
        if (totalPage.value - current.value <= 4) {
          nextDot.value = false;
          for (
            let i = totalPage.value - displayPage.value + 1;
            i <= totalPage.value;
            i++
          ) {
            pageList.value.push(i);
