...
export default function findCityZipCode(zipCode: string | number): string {
  const item = cityZipCode.find(item => Number(item.code) === Number(zipCode));

  if (item && item.name.includes('省')) {
    item.name = item.name.replace('省', '');
  }

  if (item && item.name.includes('市')) {
